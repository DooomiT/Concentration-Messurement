...
class ConcentrationEvaluator:
    ...
    def __init__(self, shared_queue, file_path, first_average_time=10):
    ...
    def evaluateAverageKeys(self, keys):
        amount_of_adding_keys, amount_of_deleting_keys = self.getAmountAddingDeletingKeys(
            keys)
    ...
    def evaluateFirstConcentration(self, shared_queue):
        self.log(logging.debug, "evaluating first average for {} seconds".format(
            self.first_average_time))
        ...
        self.log(logging.debug, "evaluated first - average = {}, variance = {}".format(
            self.first_average, self.first_variance))
    ...
    def evaluateNewConcentration(self, shared_queue):
        ...
        self.log(logging.debug, "evaluated std - average = {}, variance = {}".format(
            current_average, current_variance))
        self.current_concentration = {}
        self.current_concentration["timestamp"] = time.time()
        self.current_concentration["concentration"] = (
            self.first_average - current_average) + (self.first_variance - current_variance)
        self.log(logging.debug, "user concentration = {}".format(
            self.current_concentration))

    def run(self, shared_queue):
        while(self.running):
            if(self.first_average == None):
                self.evaluateFirstConcentration(shared_queue)
            self.evaluateNewConcentration(shared_queue)
            with open(self.file_path, 'a+') as output:
                output.write("{}:{}\n".format(
                    self.current_concentration["timestamp"], self.current_concentration["concentration"]))
